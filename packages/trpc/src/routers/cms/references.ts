...
export const referencesRouter = createTRPCRouter({
  /**
   * Searches a collection's entries and returns reference options (value/label
  ...
  search: cmsProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const valueTemplate = input.valueTemplate || "{path}";
        const labelTemplate = input.labelTemplate || "{name}";
        const searchFields = input.searchFields?.filter(Boolean).length
          ? input.searchFields.filter(Boolean)
          : ["name"];
        const selectedValues = (input.values ?? []).filter(Boolean);
        ...
        if (!normalizedPath)
          throw createHttpError(
            `Invalid path for collection "${input.name}".`,
            400
          );
