...
// Map an engine HttpError (or anything error-like) onto a TRPCError,
// preserving the message. Mirrors the status mapping of `toErrorResponse`.
const toTRPCError = (error: unknown): TRPCError => {
  const status = getErrorStatus(error);
  return new TRPCError({
    code: STATUS_TO_TRPC_CODE[status] ?? "INTERNAL_SERVER_ERROR",
    message: getErrorMessage(error),
    cause: error,
  });
};
