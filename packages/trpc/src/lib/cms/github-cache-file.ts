...
import { db } from "./db";
import { cacheFileMetaTable, cacheFileTable } from "./db";
...
} from "./github-cache-folders";
import { getCacheFileMeta, upsertCacheFileMeta } from "./github-cache-meta";
import { getParentPath } from "@workspace/cms-core/utils/file";
...
const BRANCH_HEAD_TTL_MS = parseInt(
  process.env.BRANCH_HEAD_TTL_MS || "15000",
  10
);
...
const cacheFileCheckTTLms =
  parseInt(CACHE_RECONCILE_INTERVAL_MIN, 10) * 60 * 1000;
...
const finalizePatchedFolderMetas = async (
  ...
  const candidatePaths = Array.from(contextByPath.keys()).filter(
    (folderPath) => !failedPaths.has(folderPath)
  );
  ...
  const keptPaths = candidatePaths.filter((folderPath) =>
    nonEmptyPaths.has(folderPath)
  );
  ...
  await Promise.all(
    keptPaths.map((folderPath) =>
      upsertScopedMeta(
        ...
        repo,
        branch,
        getFolderScope(contextByPath.get(folderPath)!, folderPath),
        {
...
const updateMultipleFilesCache = async (
  ...
  const verifiedDirectFolderContexts = commit
    ? await getVerifiedDirectFolderContexts(
        owner,
        repo,
        branch,
        directFolderPaths
  ...
  const verifiedDirectFolderPaths = Array.from(
    verifiedDirectFolderContexts.keys()
  );
  const collectionFolderPaths = verifiedDirectFolderPaths.filter(
    (folderPath) =>
      verifiedDirectFolderContexts.get(folderPath) === "collection"
  );
  ...
  const filesToProcess = [...modifiedFiles, ...addedFiles].filter((file) => {
    if (!verifiedDirectFolderContexts.has(getParentPath(file.path)))
      return false;
    ...
    if (
      existingEntry.commitTimestamp &&
      existingEntry.commitTimestamp.getTime() > commit.timestamp
    )
      return false;
    if (
      existingEntry.commitTimestamp &&
  ...
  if (filesToProcess.length > 0) {
    ...
    for (const chunk of graphqlChunks) {
      ...
      const variables = {
        ...
        repo,
        ...Object.fromEntries(
          chunk.map((file, i) => [`exp${i}`, `${branch}:${file.path}`])
        ),
      };
      ...
      try {
        ...
        console.error(
          `GraphQL query failed for chunk [${chunk.map((f) => f.path).join(", ")}]: ${error.message}`
        );
        chunk.forEach((file) =>
      ...
      for (let i = 0; i < chunk.length; i++) {
        ...
        if (!fileData) {
          console.warn(
            `Skipping cache update for ${file.path}: File data missing from GitHub response.`
          );
          failedDirectFolderPaths.add(getParentPath(file.path));
          continue;
        }
        ...
        const context =
          verifiedDirectFolderContexts.get(parentPath) ?? "collection";
        const now = new Date();
...
const updateFileCache = async (
  ...
  const changedPaths =
    operation.type === "rename" && operation.newPath
  ...
  const failedDirectFolderPaths = new Set<string>();
  const shouldPreserveSingleFolder =
    operation.commit && verifiedDirectFolderContexts.has(parentPath);
  ...
  const foldersToClaim =
    operation.type === "rename"
      ? shouldPreserveRename
        ? [parentPath]
        : []
      : shouldPreserveSingleFolder
        ? [parentPath]
        : [];
  ...
  try {
    switch (operation.type) {
      ...
      case "modify":
        if (operation.content === undefined || !operation.sha) {
          throw new Error(
            "Content and SHA are required for add/modify operations"
          );
        }
        ...
        if (shouldPreserveSingleFolder) {
          ...
          const entryData = {
            ...
            commitTimestamp: operation.commit?.timestamp
              ? new Date(operation.commit.timestamp)
              : null,
          };
    ...
    await Promise.all(
      Array.from(affectedFolderPaths).map((folderPath) =>
        markFolderScopeError(
          owner,
          repo,
          branch,
  ...
  if (foldersToClaim.length > 0) {
    ...
    keptPaths.forEach((folderPath) =>
      preservedDirectFolderPaths.add(folderPath)
    );
  }
...
const updateFileCacheOwner = async (oldOwner: string, newOwner: string) => {
  await db
    .update(cacheFileTable)
    .set({ owner: newOwner.toLowerCase() })
    .where(eq(cacheFileTable.owner, oldOwner.toLowerCase()));
};
...
const getCollectionCache = async (
  ...
  if (stableMeta?.status === "syncing") {
    const githubEntries = await fetchCollectionDirectoryEntries(
      owner,
      repo,
      branch,
      dirPath,
    ...
    if (subdirs.length > 0) {
      ...
      let nodeEntries = await db.query.hubCacheFile.findMany({
        where: and(
          ...
          inArray(
            cacheFileTable.path,
            subdirs.map((dir: any) => `${dir.path}/${nodeEntryFilename}`)
          )
        ),
      });

      // Query the GitHub API for the missing node entries
      const missingSubdirs = subdirs.filter(
        (dir: any) =>
          !nodeEntries.some(
            (entry: any) => entry.path === `${dir.path}/${nodeEntryFilename}`
          )
      );

      if (missingSubdirs.length > 0) {
        const nodeEntryExpressions = missingSubdirs.map(
          (dir: { path: string }, i: number) => ({
            alias: `nodeFile${i}`,
        ...
        const variablesNodeEntries = {
          ...
          repo,
          ...Object.fromEntries(
            nodeEntryExpressions.map((nf) => [`exp${nf.alias}`, nf.expression])
          ),
        };

        const responseNodeEntries: any = await octokit.graphql(
          queryNodeEntries,
          variablesNodeEntries
        );
...
const getMediaCache = async (
  ...
  const hasVerifiedSnapshot =
    !!nocache || hasVerifiedFolderSnapshot(stableMeta, branchMeta);
  ...
  if (!nocache && stableMeta?.status === "syncing") {
    const githubEntries = await fetchMediaDirectoryEntries(
      owner,
      repo,
      branch,
      path,
