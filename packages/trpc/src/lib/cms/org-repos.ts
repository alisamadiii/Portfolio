...
/**
 * Delete every project-scoped row (domains, deployments, blog posts,
 * subscription, collaborators) for the given repos — but NOT the hub_project
 * rows themselves. App-level cascade — the hub DB has no FK cascade because
 * hub_project.repo_id is a unique index, not a constraint.
 */
...
/**
 * Delete a project and ALL its project-scoped rows, then the hub_project row
 * itself. Used by the org reconcile to drop repos removed from GITHUB_ORG. The
 * Danger tab tombstones instead (keeps the row hidden), so it uses
 * deleteProjectChildRows directly.
 */
const deleteProjectsCascade = async (repoIds: number[]) => {
  if (!repoIds.length) return;
  await deleteProjectChildRows(repoIds);
  await db.delete(hubProject).where(inArray(hubProject.repoId, repoIds));
};

export {
  deleteProjectChildRows,
  deleteProjectsCascade,
  fetchOrgRepos,
  getGithubEnv,
  syncOrgRepos,
};
