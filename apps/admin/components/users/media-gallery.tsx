...
export function MediaGallery({ userId }: { userId: string }) {
  ...
  const listOptions = trpc.uploads.list.queryOptions({ prefix });
  ...
  const handleDelete = () => {
    if (!deleteKey) return;
    deleteFile.mutate({ key: deleteKey });
  };
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteKey ? fileName(deleteKey) : ""}&quot;? This action cannot
              be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            {files.map((file) =>
              isImage(file.key) ? (
                ...
                >
                  ...
                  <div className="bg-background/80 absolute right-0 bottom-0 left-0 px-2 py-1">
