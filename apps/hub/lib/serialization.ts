...
const parse = (
  ...
  const match = isDefaultJsonFrontmatter
    ? content.match(/^(\{[\s\S]*\})\n*([\s\S]*)$/)
    : (() => {
        const startDelimiter = delimiters[0].replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&"
        );
        const endDelimiter = delimiters[1].replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&"
        );
  ...
  contentObject = deserialize(
    frontMatter,
    format.split("-")[0] as SerialFormat
  );
...
const stringify = (
  ...
  // YAML/JSON/TOML without frontmatter
  if (["yaml", "json", "toml"].includes(format))
    return serialize(contentObject, format as SerialFormat);
...
const setDelimiter = (
  delimiters: string | [string, string] | null | undefined,
