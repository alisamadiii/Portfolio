  ...
  const content = (
    ...
    >
      ...
      {selectable && isSelected && (
        <div className="text-primary-foreground bg-primary absolute top-2 left-2 rounded-full p-0.5">
...
const MediaView = ({
  ...
  const mediaConfig = useMemo(() => {
    ...
    return config.object.media.find(
      (item: { name: string }) => item.name === media
    );
    ...
    (entry: FileSaveData) => {
      ...
      setData((prevData) => {
        ...
        const existingIndex = prevData.findIndex(
          (item) => item.path === mediaEntry.path
        );
        if (existingIndex >= 0) {
          const next = [...prevData];
  ...
  const handleFolderCreate = useCallback(
    (entry: unknown) => {
      ...
      setData((prevData) => {
        ...
        if (
          prevData.some(
            (item) => item.path === parent.path && item.type === "dir"
          )
        ) {
          return prevData;
        }
  ...
  const breadcrumbNode = useMemo(() => {
    const breadcrumbTextClass = usePageHeader
      ? "font-semibold text-lg flex-nowrap"
      : "text-sm flex-nowrap";
  ...
  useOptionalRepoHeader({ header: headerNode }, { enabled: usePageHeader });
        ...
        <div className="flex flex-1 flex-col space-y-4">
          <header className="flex items-center justify-between gap-x-2">
            <div className="sm:flex-1">
              <div className="hidden min-w-0 truncate overflow-hidden sm:block">
                {breadcrumbNode}
              </div>
              <Button
