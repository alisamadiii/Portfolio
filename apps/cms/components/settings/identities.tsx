...
export function Identities({
  ...
  const handleConnectGithub = async () => {
    ...
    try {
      // Auth is hosted by the portfolio app — relative callbacks would land
      // on its origin, so send absolute URLs back to the CMS.
      const result = await signIn.social({
        provider: "github",
        callbackURL: `${urls.cms}/settings`,
        errorCallbackURL: `${urls.cms}/settings`,
      });
  ...
  const handleDisconnectGithub = async () => {
    ...
    try {
      ...
      if (error) {
        throw new Error(error.message || "Failed to disconnect GitHub account.");
      }
