    ...
    <>
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
          ...
          <div className="hidden items-center gap-9 md:flex">
            {["services", "about", "pricing", "contact"].map((s) => (
              <a key={s} href={`#${s}`} className="nav-link text-[13px] capitalize text-muted transition-colors duration-300 hover:text-text">{s}</a>
            ))}
            <motion.a
              ref={magnetic.ref}
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-10 bg-bg">
          <button className="absolute top-6 right-6 text-text" onClick={() => setMenuOpen(false)} aria-label="Close">
