    ...
    >
      <DeepDiveRow
        ...
        code={
          <>
            ...
            <span style={{ color: W }}>async</span> ({"{ data }"}) =&gt;{"\n"}
      ...
      <DeepDiveRow
        ...
        code={
          <>
            <span style={{ color: C }}>
              // admin routes guarded by session + role
            </span>
            {"\n"}
            <span style={{ color: W }}>const</span> user ={" "}
            <span style={{ color: W }}>await</span> auth.api.getSession({"{"}
            {"\n"}
            {"  "}headers: <span style={{ color: W }}>await</span> headers(),
            {"\n"}
            {"});"}
            {"\n"}
            <span style={{ color: W }}>if</span> (!user) redirect(
            <span style={{ color: S }}>"/login"</span>);{"\n"}
            <span style={{ color: W }}>if</span> (user.user.role !=={" "}
            <span style={{ color: S }}>"admin"</span>) notFound();
          </>
        }
      />
      <DeepDiveRow
        ...
        code={
          <>
            <span style={{ color: C }}>
              // drop a folder in services/, add a tRPC router, done
            </span>
            {"\n"}
            <span style={{ color: W }}>export const</span> teamsRouter =
            createTRPCRouter({"{"}
            {"\n"}
