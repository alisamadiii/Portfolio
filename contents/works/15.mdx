...
function TodoItem({
  ...
  return (
    <motion.div layout className="flex min-h-7 items-center gap-2">
      <motion.button
        className="border-natural-300 bg-natural-200 text-natural-600 flex h-4 w-4 items-center justify-center rounded-sm border"
        style={{ x }}
        onClick={() => {
          setTodos((prev) =>
        ...
        {show === todo.id && (
          <div className="ml-auto flex items-center gap-px">
            <motion.button
              ...
              className="hover:bg-natural-150 rounded-md p-1 transition-colors"
              onClick={() => {
                setTodos((prev) =>
                  prev.map((t) =>
                    t.id === todo.id ? { ...t, completed: true } : t
