...
```bash title="Terminal"
pnpm i contentlayer next-contentlayer
```

These are the packages that will be used for styling:

```bash title="Terminal"
pnpm i rehype-autolink-headings rehype-pretty-code rehype-slug remark-gfm@^3.0.1
```
...
Make sure your file type is `.js` not `.mjs`, and add the following code:
...
```json title="tsconfig.ts" showLineNumbers {10}#add
"jsx": "preserve",
"incremental": true,
"plugins": [
  {
...
Working with contentlayer, naming file is crucial. I will use `docs` as the folder name.
...
This is how the data going to look like:
...
I am going to name my file, `installation.mdx`.
...
Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis eos impedit iste reprehenderit veritatis vitae itaque ab accusantium vel? Unde debitis tenetur explicabo totam veniam dolorum soluta, at quam rem?

## Examples

Lorem ipsum dolor sit amet `consectetur` adipisicing elit. Facilis eos impedit iste reprehenderit veritatis vitae itaque ab `accusantium` vel? Unde debitis tenetur explicabo totam veniam dolorum soluta, at quam rem?

```bash title="tailwind.config.ts"
pnpm i
```
...
```ts title="tailwind.config.ts" showLineNumbers {5}#remove /mono: ["var(--font-geist-mono)"],/#add
...
theme: {
    extend: {
      fontFamily: {
...
```tsx title="[...docs]/layout.tsx" showLineNumbers
import React from "react";
...
export default function DocsLayout({ children }: Props) {
  return (
    <div className="mx-auto flex w-full max-w-7xl">
      <Sidebar />
      <main className="w-full overflow-hidden py-8 md:px-6">
        {children}
        <ChangingDocs />
      </main>
      <RightSidebar />
...
```tsx title="[...docs]/page.tsx" showLineNumbers
import React from "react";
...
There are two files, `Sidebar.tsx` and `RightSidebar.tsx`, that you need to use them in `layout.tsx`.
...
const sortNames = ["requirements", "mdx-setup", "auth", "third-party"];

export function Sidebar() {
  const { docs } = useParams<{ docs: string[] }>();

  const transformedRoutes = transformSlugs(
    allDocs
      .sort((a, b) => {
        ...
        const indexA = sortNames.includes(baseA)
          ? sortNames.indexOf(baseA)
          : sortNames.length;
        const indexB = sortNames.includes(baseB)
          ? sortNames.indexOf(baseB)
          : sortNames.length;
  ...
  return (
    <div className="sticky top-0 hidden h-screen w-[284px] flex-shrink-0 py-8 md:block">
      <ul>
        {transformedRoutes.map((doc) => {
          return Object.entries(doc).map(([value, path], index) => {
            return typeof path === "string" ? (
              <li key={index}>
                <Link
                  href={path}
                  className={cn(
                    "inline-block h-[28px] w-full text-sm text-[#666]",
                    path === `/${docs.join("/")}` && "text-primary"
                  )}
                >
                  {allDocs.find((doc) => doc.slug === path)?.title}
                </Link>
              </li>
            ) : (
              <li className="my-4">
                ...
                <ul className="mt-2 pl-4">
                  {Object.entries(path).map(([value, path], index) => {
                    return (
                      <li key={index} data-label={value}>
                        <Link
                          href={path as string}
                          className={cn(
                            "inline-block h-[28px] w-full text-sm text-[#666]",
...
export function RightSidebar() {
  ...
  const findingDocs = allDocs.find((doc) => doc.slug === `/${docs.join("/")}`);

  return (
    <ul className="sticky top-0 hidden h-screen w-[280px] flex-shrink-0 py-8 xl:block">
...
Make sure to have the [`cn`](/requirements#cn), because we will use it a lot.

### MDX Content

```tsx title="components/MDXContent.tsx" showLineNumbers
"use client";
...
If you are using eslint, make sure to ignore the `MDXContent.tsx`:
...
```tsx title="components/Tabs.tsx" showLineNumbers
...
```tsx title="components/FilesTab.tsx" showLineNumbers
import React, { useState, ReactNode } from "react";
...
```js title="scripts/build-registry.js" showLineNumbers
...
```json title="package.json" showLineNumbers{5} {7}#add
"scripts": {
  "dev": "next dev",
  "build": "next build",
  "start": "next start",
  ...
  "build:registry": "node ./src/scripts/build-registry.js && prettier --write ./src/__registry__/index.tsx"
},
```

```tsx title="__registry__/index.tsx" showLineNumbers
// @ts-nocheck
import * as React from "react";
...
export const Index: Record<string, any> = {
  hotKeys: {
    name: "hotKeys",
    component: dynamic(() => import("@/preview/hotKeys.tsx"), { ssr: false }),
  },
  "input-otp": {
    name: "input-otp",
    component: dynamic(() => import("@/preview/input-otp.tsx"), { ssr: false }),
  },
  "react-pdf": {
    name: "react-pdf",
    component: dynamic(() => import("@/preview/react-pdf.tsx"), { ssr: false }),
  },
  "use-click-outside": {
    name: "use-click-outside",
    component: dynamic(() => import("@/preview/use-click-outside.tsx"), {
      ssr: false,
    }),
  },
  "use-debounced-state": {
    name: "use-debounced-state",
    component: dynamic(() => import("@/preview/use-debounced-state.tsx"), {
      ssr: false,
    }),
  },
  "use-fullscreen": {
    name: "use-fullscreen",
    component: dynamic(() => import("@/preview/use-fullscreen.tsx"), {
      ssr: false,
    }),
  },
  "use-media-query": {
    name: "use-media-query",
    component: dynamic(() => import("@/preview/use-media-query.tsx"), {
      ssr: false,
    }),
  },
  uuid: {
    name: "uuid",
    component: dynamic(() => import("@/preview/uuid.tsx"), { ssr: false }),
  },
};
```

```tsx title="components/PreviewComponent.tsx" showLineNumbers
"use client";
...
Create two new folders, and add the following file:

```bash title="Terminal"
mkdir src/preview src/__registry__
```

```tsx title="preview/button.tsx"
import React from "react";
...
```tsx title="components/ChangingDocs.tsx" showLineNumbers
"use client";
...
export default function ChangingDocs() {
  ...
  const docsIndx = allDocs.findIndex(
    (doc) => doc.slug === `/${docs.join("/")}`
  );
  ...
  return (
    <div className="mt-8 flex justify-between border-t-wrapper pt-6 text-muted">
      {allDocs[docsIndx - 1] && (
        <Link
          href={allDocs[docsIndx - 1].slug}
          className="hover:text-foreground flex flex-col gap-1"
        >
          <span className="text-sm">Previous</span>
          {allDocs[docsIndx - 1].title}
        </Link>
      )}
      {allDocs[docsIndx + 1] && (
        <Link
          href={allDocs[docsIndx + 1].slug}
          className="hover:text-foreground ml-auto flex flex-col items-end gap-1"
        >
...
```tsx title="components/CopyButton.tsx" showLineNumbers
...
```tsx title="components/TechIcons.tsx" showLineNumbers
import React from "react";
...
```css title="global.css" showLineNumbers{5}
@tailwind base;
@tailwind components;
@tailwind utilities;
...
```bash title="Terminal"
pnpm build:registry && pnpm dev
```
> Make sure to refresh your VSCode to not seeing any TS errors.
